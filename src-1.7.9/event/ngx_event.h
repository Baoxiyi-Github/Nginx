...
struct ngx_event_s {
    ...
    unsigned         timedout:1;    //用于标识当前事件是否已经超时
    unsigned         timer_set:1;   //用于标识当前事件是否已经是否加入到红黑树管理，需要对其是否超时做监控
    ...
    //红黑树节点类型变量，红黑树就算通过该字段来组织所有的超时事件对象
...
typedef struct {
    ...
    //选用的事件模块在所有事件模块中的序号， 即ngx_cycle_t中ctx_index成员
    ngx_uint_t    use;

    //标志位，如果为1，则表示在接受到一个新连接事件时，一次性建立尽可能多的连接
